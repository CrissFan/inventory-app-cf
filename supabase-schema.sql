...
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_team_id UUID; v_role TEXT; v_item JSONB; v_quantity NUMERIC; v_material_id BIGINT;
  v_cut_quantity INTEGER; v_factory_result JSONB; v_material_result JSONB;
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(p_material_consumptions) item GROUP BY item->>'material_id' HAVING COUNT(*) > 1)
  ...
  FOR v_item IN SELECT value FROM jsonb_array_elements(p_material_consumptions) ORDER BY (value->>'material_id')::BIGINT LOOP
    ...
    IF NOT EXISTS (SELECT 1 FROM inventory_material_product_links WHERE team_id = v_team_id AND material_id = v_material_id AND product_id = p_product_id)
    ...
    v_material_result := record_material_consumption(
      ...
      LEFT('待出货录入' || CASE WHEN BTRIM(COALESCE(p_note, '')) = '' THEN '' ELSE '：' || BTRIM(p_note) END, 500), NULL
    );
