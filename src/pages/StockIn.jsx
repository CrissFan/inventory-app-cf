...
import { ArrowDownToLine, ArrowLeft, Search, ScanLine, Check, Package, ChevronRight, ListPlus } from 'lucide-react';
import { getProducts, stockIn, getProductByBarcode, getTags } from '../api/client';
...
import BatchStockIn from '../components/BatchStockIn';
...
export default function StockIn({ initialSelection = null }) {
  const { user } = useAuth();
  ...
  const [variantLockLabel, setVariantLockLabel] = useState('尺码已锁定');
  const [quantity, setQuantity] = useState(1);
  const [reason, setReason] = useState('退换货入库');
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (!initialSelectionAppliedRef.current && initialSelectionRef.current?.productId != null) {
        const target = initialSelectionRef.current;
        const product = list.find(item => String(item.id) === String(target.productId));
        const variant = product?.variants?.find(item => String(item.id) === String(target.variantId));
        if (product && variant) {
          setSelected(product);
          setSelectedVariant(variant);
          setVariantLocked(true);
          setVariantLockLabel(target.source === 'low-stock-alert' ? '预警尺码已锁定' : '尺码已锁定');
  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-16 px-4">
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-1">入库成功</h2>
        ...
        <p className="text-2xl font-bold text-green-600 mb-6">+{success.quantity} {success.product?.unit || '个'}</p>
        <p className="text-sm text-gray-400 mb-6">该尺码库存: {success.currentStock} {success.product?.unit || '个'}</p>
        <div className="flex gap-3">
          <button onClick={() => setSuccess(null)} className="btn-secondary">继续入库</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => { setSelected(null); setSelectedVariant(null); setVariantLocked(false); setVariantLockLabel('尺码已锁定'); }}
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">增加商品库存</p>
          </div>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1">
              ...
              <button onClick={() => { setSelected(null); setSelectedVariant(null); setVariantLocked(false); setVariantLockLabel('尺码已锁定'); }} className="text-xs text-primary-600 hover:underline">更换</button>
            </div>
          </div>
        </div>
        ...
        <div className="card p-5 space-y-4">
          <VariantSelector variants={selected.variants} value={selectedVariant} onChange={setSelectedVariant} tone="green" locked={variantLocked} lockedLabel={variantLockLabel} />
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">入库数量</label>
            ...
            <div className="flex gap-2 mt-2">
              {[5, 10, 20, 50, 100].map(n => (
                <button key={n} onClick={() => setQuantity(n)} className="px-3 py-1 text-xs rounded-md bg-gray-100 text-gray-600 hover:bg-gray-200">{n}</button>
              ))}
            </div>
          </div>
          ...
          <div className="pt-2">
            <button onClick={handleSubmit} disabled={submitting} className="btn-success w-full">
              <ArrowDownToLine className="w-4 h-4" />
              {submitting ? '提交中...' : `确认入库 ${quantity}`}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900">入库</h1>
          <p className="text-sm text-gray-500 mt-0.5">选择要入库的商品</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowBatchStockIn(true)} className="btn-secondary px-3">
      ...
      {loading ? (
        ...
        <div className="card divide-y divide-gray-50">
          {displayProducts.map(p => (
            <button
              key={p.id}
              onClick={() => { setSelected(p); setSelectedVariant(p.variants?.length === 1 ? p.variants[0] : null); setVariantLocked(false); setVariantLockLabel('尺码已锁定'); }}
