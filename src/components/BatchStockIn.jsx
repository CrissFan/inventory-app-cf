...
import { AlertCircle, ArrowLeft, CheckCircle2, Download, FileSpreadsheet, LoaderCircle, Package, Search, Upload } from 'lucide-react';
...
export default function BatchStockIn({ onClose, onCompleted }) {
  const { user } = useAuth();
  ...
  const rows = useMemo(() => products.flatMap(product => (product.variants || []).map(variant => ({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col bg-gray-50">
      <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-gray-100 bg-white px-4 py-3" style={{ paddingTop: 'max(12px, env(safe-area-inset-top))' }}>
        ...
        <div className="min-w-0 flex-1"><h2 className="font-bold text-gray-900">批量入库</h2><p className="truncate text-xs text-gray-400">选择商品尺码或上传入库表格</p></div>
        <input ref={fileInputRef} type="file" accept=".xlsx,.xls,.csv" className="hidden" onChange={event => importSheet(event.target.files?.[0])} />
      ...
      <main className="flex-1 overflow-y-auto p-4">
        <div className="mx-auto max-w-6xl space-y-4 pb-32">
          <section className="card p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-gray-400">“导出表格”默认导出全部商品尺码；有勾选时仅导出所选项。填写“入库数量”后，可上传同一格式的 Excel 或 CSV 文件。</p>
          </section>
          ...
          <section className="card overflow-hidden">
            {loading ? <div className="flex h-48 items-center justify-center"><LoaderCircle className="h-7 w-7 animate-spin text-primary-500" /></div> : filteredRows.length === 0 ? <div className="p-12 text-center text-sm text-gray-400"><Package className="mx-auto mb-2 h-9 w-9 text-gray-300" />{search ? '未找到匹配的商品尺码' : '暂无可入库商品'}</div> : <div className="overflow-x-auto">
      ...
      <footer className="shrink-0 border-t border-gray-200 bg-white px-4 py-3" style={{ paddingBottom: 'max(12px, env(safe-area-inset-bottom))' }}>
        <div className="mx-auto flex max-w-6xl items-center gap-3"><div className="min-w-0 flex-1 text-xs text-gray-500"><p>已选 {selectedRows.length} 个商品尺码</p><p className="mt-0.5 truncate">合计入库 {totalQuantity} 件 · {reason || '未选择原因'}</p></div><button type="button" onClick={submit} disabled={submitting || !selectedRows.length} className="btn-success min-w-36 px-5">{submitting ? <><LoaderCircle className="h-4 w-4 animate-spin" />提交中...</> : <><FileSpreadsheet className="h-4 w-4" />确认批量入库</>}</button></div>
      </footer>
    </div>
  );
}
